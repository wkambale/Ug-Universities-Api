...
async def get_stats(db: AsyncSession = Depends(get_db)):
    ...
    active_q = select(func.count(University.id)).filter(University.is_active == True)
    ...
    # By Type
    type_q = select(University.type, func.count()).filter(University.is_active == True).group_by(University.type)
    type_res = await db.execute(type_q)
    by_type = {row[0]: row[1] for row in type_res.all()}
    
    # By Location
    loc_q = select(University.location, func.count()).filter(University.is_active == True).group_by(University.location).order_by(func.count().desc()).limit(10)
